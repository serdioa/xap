...
public class TieredStorageUtils {
    ...
    public static IEntryHolder getEntryHolderFromRow(IServerTypeDesc serverTypeDesc, ResultSet resultSet) throws SQLException {
        ...
        if (typeDesc.isAutoGenerateId()) {
            // autogenerated id is always a single property
            uid = (String) data.getFixedPropertyValue(((PropertyInfo) typeDesc.getFixedProperty(idPropertiesNames.get(0))).getOriginalIndex());
        } else {
            Object id = TypeDescriptorUtils.toSpaceId(idPropertiesNames, s -> data.getFixedPropertyValue(((PropertyInfo) typeDesc.getFixedProperty(s)).getOriginalIndex()));
