...
public class NotificationReplicationChannelDataFilter extends ReliableAsyncChannelDataFilter {
    ...
    private boolean filterSingleEntry(ServerEntry entry, ReplicationSingleOperationType operationType) {
        ...
        if (getTemplate().isAssignableFrom(typeDesc)) {
            if (getTemplate().match(_cacheManager.getCacheContext(), _cacheManager, entry, -1, null, _regexCache, -1))
                return true;
        }
        return false;
    ...
    private MatchLevel filterUpdateEntry(ServerEntry entry, ServerEntry prevEntry, Logger contextLogger, ReplicationSingleOperationType operationType) {
        ...
        if (getTemplate().isAssignableFrom(typeDesc)) {
            if (!matchPrevious && prevEntry != null && getTemplate().match(_cacheManager.getCacheContext(), _cacheManager, prevEntry, -1, null, _regexCache, -1))
                matchPrevious = true;
            if (!matchCurrent && getTemplate().match(_cacheManager.getCacheContext(), _cacheManager, entry, -1, null, _regexCache, -1))
                matchCurrent = true;
        }
