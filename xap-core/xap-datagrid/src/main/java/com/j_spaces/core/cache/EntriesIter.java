...
public class EntriesIter extends SAIterBase implements ISAdapterIterator<IEntryHolder> {
    ...
    private static Logger logger = LoggerFactory.getLogger(EntriesIter.class.getName());
    ...
    public EntriesIter(Context context, ITemplateHolder template, IServerTypeDesc serverTypeDesc,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Init EntriesIter for template [" + template+ "]");
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("EntriesIter created - memoryOnly=" + _memoryOnly +", doneWithCache=" + _doneWithCache);
        }
    }
